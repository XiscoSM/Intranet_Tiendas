...
/// <summary>
/// Migración de IntranetTiendas/PedidosCentralLin_Alm.asp (detalle resumen por almacén central).
/// Procedure: asp.PedidoCentral_Central_ResumDetall_Select (@Empresa, @Fecha, @AlmCentral, @Estado).
/// P2 = fecha, P3 = estado, P4 = filtro de vista (3 = solo líneas con diferencias).
/// @AlmCentral era la cookie ALMACEN: ahora Alm de la sesión.
/// </summary>
public class PedidosCentralLinAlmModel(DbService db) : BasePageModel(db)
{
    ...
    private long Estado => long.TryParse(P3, out var e) ? e : 0;

    public async Task OnGetAsync()
    {
        Lineas = await Db.QueryProcAsync("asp.PedidoCentral_Central_ResumDetall_Select",
    ...
    {
        var rows = await Db.QueryProcAsync("asp.PedidoCentral_Central_ResumDetall_Select",
            new { Empresa, Fecha, AlmCentral = Alm, Estado });
        return Excel(rows, "PedidosCentralLinAlm");
    }
}
